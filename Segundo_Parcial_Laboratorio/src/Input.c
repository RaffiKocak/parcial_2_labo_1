...
int PedirCadenaConReintentos(char cadenaADevolver[], int tamCadenaADevolver, char mensaje[], int intentos) {
	...
	while (intentos > 0 && (GetString(cadenaAVerificar, sizeof(cadenaAVerificar)) == -1)) {
			printf("ERROR! %s", mensaje);
			intentos--;
	}

	if (intentos > 0) {
		EmprolijarNombre(cadenaAVerificar);
		strncpy(cadenaADevolver, cadenaAVerificar, sizeof(cadenaAVerificar));
	} else {
		printf("\nDemasiados intentos fallidos. Ingreso de dato cancelado.\n");
		retorno = 0;
	}
